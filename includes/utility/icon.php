...
function get_icon_sets(): array {
	$options = get_option( 'alexa' )['iconSets'] ?? [
		[
			'label' => 'WordPress',
			'value' => 'wordpress',
		],
	...
	return apply_filters( 'alexa_icon_sets', $icon_sets );
}
...
function get_icon( string $set, string $name, int $size = null ): string {
	...
	$label = ucwords( str_replace( '-', ' ', $name ) ) . __( ' Icon', 'alexa' );
...
function get_icon_html( string $content, array $block ): string {
	...
	$aria_label = $img->getAttribute( 'alt' ) ? $img->getAttribute( 'alt' ) : str_replace( '-', ' ', $icon_name ) . __( ' icon', 'alexa' );
