...
function get_conditional_stylesheets( string $content, bool $is_editor ): string {
	...
	$conditions['block-styles'] = [
		...
		'mobile-center'    => str_contains( $content, 'is-style-mobile-center' ),
		'float'    => str_contains($content, 'is-style-float'),
		'sink'    => str_contains( $content, 'is-style-sink' ),
		'grow'    => str_contains($content, 'is-style-grow'),
...
add_action( 'alexa_editor_scripts', NS . 'enqueue_editor_only_styles' );
/**
...
function add_editor_stylesheets() {
	...
	add_editor_style( 'https://alexa-dynamic-styles' );
}
...
function generate_dynamic_styles( $response, array $parsed_args, string $url ) {
	if ( $url === 'https://alexa-dynamic-styles' ) {
		$response = [
...
add_filter( 'alexa_inline_css', NS . 'add_child_theme_style_css' );
/**
