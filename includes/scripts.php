...
function add_editor_scripts_hook( WP_Screen $screen ): void {
	...
	add_action(
		...
		static fn() => do_action( 'alexafse_editor_scripts', $screen )
	);
}

add_action( 'alexafse_editor_scripts', NS . 'enqueue_editor_scripts' );
/**
...
function get_editor_data() {
	...
	return apply_filters(
		'alexafse_editor_data',
		[
			'url'           => get_uri(),
			'siteUrl'       => trailingslashit( home_url() ),
			'ajaxUrl'       => admin_url( 'admin-ajax.php' ),
			'adminUrl'      => trailingslashit( admin_url() ),
			'nonce'         => wp_create_nonce( 'alexafse' ),
			'icon'          => get_icon( 'social', 'alexafse' ),
